...
const startServer = async () => {
  ...
  const channelNameToIds = (req, name, params) => new Promise((resolve, reject) => {
    ...
    case 'room':
      if (!params.room) {
        reject(new RequestError('Missing room name parameter'));
        return;
      }

      authorizeRoomAccess(params.room, req).then(() => {
        resolve({
          channelIds: [`timeline:room:${params.room}`],
          options: { needsFiltering: false },
        });
      }).catch(() => {
        reject(new AuthenticationError('Not authorized to stream this room'));
      });
  ...
  const streamNameFromChannelName = (channelName, params) => {
    ...
    } else if (channelName === 'room' && params.room) {
      return [channelName, params.room];
