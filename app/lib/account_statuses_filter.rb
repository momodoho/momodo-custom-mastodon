...
class AccountStatusesFilter
  ...
  def results
    ...
    # momodo: room (group-message) posts live only in their room timeline and
    # must never appear on a profile — exclude them from all account timelines.
    scope.where(room_id: nil)
  end
