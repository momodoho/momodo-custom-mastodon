...
module ApplicationHelper
  ...
  def render_initial_state
    ...
    if user_signed_in? && current_user.functional?
      ...
      state_params[:account_ring]      = account_ring_for_initial_state
    end
  ...
  # Display data for every account in this browser's multi-account ring. Tokens
  # are never included — switching happens server-side by account id against the
  # signed ring cookie.
  def account_ring_for_initial_state
    ids = AccountRing.read(cookies)
    ids = [current_session.session_id] if ids.blank? && current_session

    active_session_id = current_session&.session_id
    sessions_by_id    = SessionActivation.where(session_id: ids).includes(user: :account).index_by(&:session_id)

    # An account can have several sessions on one browser (e.g. re-login),
    # so collapse to one entry per account, preferring the active session.
    by_account = {}

    ids.each do |session_id|
      account = sessions_by_id[session_id]&.user&.account
      next if account.nil?

      active = session_id == active_session_id
      entry  = by_account[account.id]
      ...
      if entry
        ...
        by_account[account.id] = {
          ...
          acct: account.local? ? account.username : account.acct,
