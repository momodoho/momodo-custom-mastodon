...
class Status < ApplicationRecord
  ...
  belongs_to :room, optional: true # momodo: member-only room channel binding
  ...
  def increment_counter_caches
    # momodo: room (group message) posts are excluded from the profile post count
    # and last_status_at, exactly like DMs (direct).
    return if direct_visibility? || room_id.present?
  ...
  def decrement_counter_caches
    return if direct_visibility? || room_id.present? || new_record?
