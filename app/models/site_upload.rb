...
class SiteUpload < ApplicationRecord
  ...
  STYLES = {
    ...
    # momodo: admin-uploadable brand images — logo, login/landing background, and
    # the in-app (timeline) background — per theme (마스토돈 = default / 트위터 =
    # birdui) × color scheme (light/dark). Stored as-is. Keys look like
    # `logo_birdui_dark`, `app_bg_default_light`, etc. (12 total).
    %w(logo login_bg app_bg).product(%w(default birdui), %w(light dark)).to_h do |asset, theme, scheme|
      [:"#{asset}_#{theme}_#{scheme}", {}.freeze]
    end
  ).freeze
