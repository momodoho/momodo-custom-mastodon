...
class InstancePresenter < ActiveModelSerializers::Model
  ...
  # momodo: admin-uploadable brand images, per theme (default/birdui) × scheme
  # (light/dark) — e.g. logo_birdui_dark, app_bg_default_light (12 accessors).
  Form::AdminSettings::BRAND_UPLOAD_KEYS.map(&:to_s).each do |key|
    define_method(key) do
      ivar = :"@#{key}"
      return instance_variable_get(ivar) if instance_variable_defined?(ivar)

      instance_variable_set(ivar, Rails.cache.fetch("site_uploads/#{key}") { SiteUpload.find_by(var: key) })
    end
  end
end
