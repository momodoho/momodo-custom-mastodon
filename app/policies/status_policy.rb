...
class StatusPolicy < ApplicationPolicy
  def show?
    ...
    # momodo: moderators/admins can always view statuses.
    return true if role.can?(:manage_users, :manage_reports)

    if record.room_id?
      # momodo: room posts are visible only to the room's current members
      # (live membership check — no mentions/tags needed).
      owned? || room_member?
  ...
  def room_member?
    ...
    record.room&.member?(current_account)
  end
