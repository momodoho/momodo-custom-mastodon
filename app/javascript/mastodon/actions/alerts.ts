...
export const showAlertForError = (error: unknown, skipNotFound = false) => {
  if (error instanceof AxiosError && error.response) {
    ...
    // momodo: closed community — anonymous visitors hit 401 on
    // authenticated-only endpoints (e.g. the about page). That's expected, so
    // don't surface it as an error toast.
    if (status === 401 && !me) {
      return ignoreAlert();
    }
