...
class PostStatusService < BaseService
  ...
  def call(account, options = {})
    ...
    @room          = @options[:room] # momodo: member-only room channel
  ...
  def preprocess_attributes!
    ...
    @visibility   = :limited if @room # momodo: room posts are always limited (member-only)
  ...
  def process_status!
    raise Mastodon::ValidationError, I18n.t('rooms.errors.not_a_member') if @room && !@room.member?(@account)
    raise Mastodon::ValidationError, I18n.t('rooms.errors.images_only') if @room && @media.any?(&:audio_or_video?)

    @status = @account.statuses.new(status_attributes)
    # momodo: room posts carry no mentions (the @-mention feature is disabled
    # inside rooms) — so no notifications, and audience is the room, not tags.
    unless @room
    ...
    attach_quote!(@status) unless @room
  ...
  def postprocess_status!
    return room_postprocess_status! if @room
  ...
  # momodo: room posts bypass home/list fan-out, federation, trends, hashtags and
  # e-mail. They are delivered ONLY to the room's real-time stream (members who
  # have the room column open), and read back via the DB-backed room timeline.
