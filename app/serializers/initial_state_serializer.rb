...
class InitialStateSerializer < ActiveModel::Serializer
  ...
  def meta
    ...
    store[:account_ring] = object.account_ring if object.account_ring.present?
  ...
  def default_meta_store
    {
      ...
      custom_logo_light: instance_presenter.public_send("logo_#{brand_theme_token}_light")&.file&.url,
      custom_logo_dark: instance_presenter.public_send("logo_#{brand_theme_token}_dark")&.file&.url,
  ...
  # The active user's theme, mapped to its brand-key token (마스토돈 = default /
  # 트위터 = birdui), so the SPA logo matches the theme it renders in.
  def brand_theme_token
    theme = object.current_account&.user&.setting_theme
    theme = Setting.theme unless Themes.instance.names.include?(theme)
    theme == 'bird-ui' ? 'birdui' : 'default'
  end
