...
Warden::Manager.after_set_user except: :fetch do |user, warden|
  ...
  # Register the now-active session in this browser's multi-account ring.
  AccountRing.add(warden.cookies, session_id)
end
...
Warden::Manager.after_fetch do |user, warden|
  ...
  if session_id && (session = user.session_activations.find_by(session_id: session_id))
    ...
    # Seed the ring for sessions created before multi-account existed.
    AccountRing.add(warden.cookies, session_id)
  else
    warden.logout
...
Warden::Manager.before_logout do |_, warden|
  # When adding another account we sign the current user out of Warden but must
  # keep their SessionActivation + token alive so they stay switchable.
  next if warden.request.env['mastodon.preserve_session']
